...
public class BookAPI {
	...
	public static Iterable<UserBooks> callAPI(String user_event_name, String user_state_initials) {
		Scanner kb = new Scanner(System.in);
		{
			...
			String event_name = toDisplay.substring(event_name_loc, event_name_loc + quote_loc);
			while (substringIndex < responseString.length()) {
				// Parsing the response for relevant data
				// getting event name
				try {
					event_name_loc = toDisplay.indexOf("name\":\"");
					event_name_loc += 7;
					event_name_remaining = toDisplay.substring(event_name_loc);
					quote_loc = event_name_remaining.indexOf("\"");
					String event_name_string = toDisplay.substring(event_name_loc, event_name_loc + quote_loc);
					if (!event_name.equals(event_name_string)) {
						// Move to the next event by updating the substringIndex
						int nextEventIndex = toDisplay.indexOf("name\":\"", event_name_loc + 1);
						if (nextEventIndex == -1) {
								break; // No more events found
						}
						toDisplay = toDisplay.substring(nextEventIndex);
